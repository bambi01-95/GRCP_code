...
oop spec_setq(oop args,oop env){
    ...
    if(association==nil){
        if(args->Symbol.value==0){
            fprintf(stderr,"undefined value:[in spec_setq]");
            exit(1);
        }
        define(car(args),value);
        return value;
    }
...
oop spec_list(oop args,oop env){
    return args;
}
// oop spec_(oop args,oop env){
//     return ;
// }
...
////////////////////////////////////////////////////////*/
oop Ldefine(oop args,oop env){
    printf("in Ldefine->");
    oop pair = assoc(car(args),env);
    printf("pair:");
    println(pair);
    if(pair==nil){
        env = pairlist(car(args),eval(cadr(args),env),env);
    }
    else{
        Cell_setD(car(args),eval(cadr(args),env));
    }
    return cadr(args);
}
...
int main()
{
    ...
    // define(newSymbol("class"),newSpecial(spec_class));
    ...
    define(newSymbol("list"),newSpecial(spec_list));
