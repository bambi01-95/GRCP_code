...
oop read(void) // read stdin and return an object, or 0 if EOF
{
    ...
    if (c == '(') { //  ()  (x)  (x . y)  (x y z)  (x y . z)
        ...
        if(Object_type(car(newlist))==Symbol){
            if(assoc(car(newlist),syntaxTable)!=nil){
	...
	switch(Object_type(exp)){
		...
		case Cell:{
            ...
            if(func==sym_func){
                oop name = car(args);
                oop value = cadr(args);
                if(car(value)!=sym_lambda){
                    fprintf(stderr,"error: not function\n");
                    exit(1);
                }
				value = eval(value,env);
                name->Symbol.value = value;
                syntaxTable = newCell(newCell(name,value),syntaxTable);
                return value;
            }
			if(func==sym_define){
				oop name = car(args);
...
int main()
{
    ...
    syntaxTable = nil;//newCell(newCell(newSymbol("asdf"),newInteger(1)),nil);     
    for (;;) { // read-print loop
        oop obj = read();
        if (!obj) break;
