...
oop read(void) // read stdin and return an object, or 0 if EOF
{
    ...
    if(c == '\''){
        oop list = read();
        return newCell(sym_quote,newCell(list,nil));                //-10
    }
    if (c == EOF) return 0;
...
oop apply(oop func,oop args,oop env){
    switch(Object_type(func)){
        ...
        case Integer:
        {
            fprintf(stderr,"ILLEGAL/Integer apply()\n");
            exit(1);
        }
        case Symbol:{
        ...
        case Cell:{
            ...
            if(car(func)==sym_lambda){
                oop result = caddr(func);
                return eval(result,env);
            }

            printf("Error cell apply\n");
        }
    }
	...
	fprintf(stderr,"ERROR: switch apply()\n");
	exit(1);
	return 0;
}
	...
	switch(Object_type(exp)){
		...
		case Cell:{
			...
			if(func==sym_define){
				...
				globals = newCell(newCell(name,value),env);
				return value;
			}
