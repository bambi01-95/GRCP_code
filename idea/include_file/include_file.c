...
oop read_file(FILE *fp) // read stdin and return an object, or 0 if EOF
{
    int c;
    c = nextchar_f(fp);
    if (isdigit(c)) { // number
...
oop prim_include(oop args,oop env){
    if(Object_type(car(args))!=Symbol){
        fprintf(stderr,"ERROR: not symbol in include\n");
        exit(1);
    }
    char* file = strcat(Symbol_name(car(args)),".txt");
	FILE * fp = NULL;
...
oop prim_exit(oop args,oop env){
    if(car(args)==nil)
        printf("Error");
    else
        print(car(args));
...
int main()
{
    ...
    // sym_syntax  = newSymbol("define-syntax");
	// sym_if     	= newSymbol("if");
    ...
    define(newSymbol("include"),newPrimitive(prim_include)); // include original
    define(newSymbol("exit"),newPrimitive(prim_exit));
