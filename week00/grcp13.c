...
oop newPrimitive(prim_t function)
{
    ...
    obj->Primitive.name = nil; //initialize
    return obj;
}
oop newSpecial(prim_t function)
{
    ...
    obj->Special.name = nil; //initialize
    return obj;
}
...
oop newClosure(oop parameters, oop expressions,oop environment){
    ...
    obj->Closure.environment = environment; //initialize
    obj->Closure.name = nil;                //initialize
    return obj;
}
...
oop pairlist(oop names,oop values,oop tail){
    if(Object_type(names)!=Cell){
        return (names==nil)
            ?tail                                   // para: (a b)
            :newCell(newCell(names,values),tail);   // para: (a b . c)
    } // allow mult para if end of list is symbol.
...
// useing (def x y) in lambda, lambda shuld have one or more para.
...
int main()
{
    ...
    define(newSymbol("=="),newPrimitive(prim_eqn));
