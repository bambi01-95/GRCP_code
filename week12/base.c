...
typedef enum { ILLEGAL, Undefined, Integer, Symbol,Primitive,Special,Lambda, Cell } type_t;
...
struct Lambda   { type_t type; oop parameter, expression;};
...
oop Closure_parameters(oop obj){
    // assert()?
    return obj->Lambda.parameter;
}

oop Closure_expressions(oop obj){
    // assert()?
    return obj->Lambda.expression;
}
...
oop newClosure(oop parameters, oop expressions){
    oop obj = newObject(Lambda);
    obj->Lambda.parameter  = parameters;
    obj->Lambda.expression = expressions;
    return obj;
}
...
void print(oop obj)
{
    ...
    case Special :  printf("%p", Special_func(obj)); return ;
    case Lambda:{
        printf("<Closure.");
        print(obj->Lambda.expression);
        printf("> <");
        print(obj->Lambda.parameter);
        putchar('>');
    }
...
oop spec_lambda(oop args,oop env){
    return newCell(sym_lambda,args);
}
...
->  int (*function)(int)
    ...
    oop function(oop args,oop env)
    oop (*function)(oop,oop)
