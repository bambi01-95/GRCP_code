...
struct Closure  { type_t type; oop name; oop parameter, expression;};
...
// 'a -> (quote a)
    ...
    if (c == '(') { //  ()  (x)  (x . y)  (x y z)  (x y . z)
        ...
        // if(Object_type(newlist)==Symbol){
        //     oop obj = newlist->Symbol.value;
        //     if(obj!=nil){
        //         printf("read");
        //         return newCell(obj,cdr(newlist));
        //     }
        // }
        return newlist;
...
void print(oop obj)
{
    ...
    case Primitive: print(obj->Primitive.name);printf("%p",Primitive_func(obj));return;
    case Special :  print(obj->Special.name);printf("%p", Special_func(obj)); return;
    case Closure:{
        printf("<Closure.");
...
oop spec_lambda(oop args,oop env){
    printf("spec_lambda\n");
    return newClosure(car(args),cdr(args));
}
...
oop apply(oop func,oop args,oop env){
    switch(Object_type(func)){
        ...
        case Closure:{
            oop newenv = pairlist(func->Closure.parameter,args,env);
